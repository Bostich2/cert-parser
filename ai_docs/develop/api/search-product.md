...
| `eaeu_ru` | OData [tech.eaeunion.org](https://tech.eaeunion.org), страна `RU` | `contains(p/productName, …)` в `technicalRegulationObjectDetails/productDetails` |
| `eaeu_other` | OData ЕАЭС, страна не `RU` | то же `productName` |
...
**32 — не лимит поля, а эвристика фразы.** Если токенов больше двух или нормализованная строка длиннее 32 символов, шаг `contains(вся строка)` пропускается: магазинский SKU в сертификате целиком не встречается и на OData/FSA даёт таймаут. Дальше OData — `contains` по 2–3 буквенным словам (размеры вроде `235/65` отбрасываются), FSA — короткая подстрока вроде бренда и линейки. Если шаг не ответил вовремя, источник пробует следующий.
