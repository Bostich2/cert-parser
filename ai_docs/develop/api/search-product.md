...
|----------|--------|-------------|
| `fsa_cert` | Росаккредитация, сертификаты (`pub.fsa.gov.ru`) | `columnsSearch.name = productFullName` («Наименование продукции»); сортировка `date` |
| `fsa_decl` | Росаккредитация, декларации | то же `productFullName`; сортировка `declDate` (не `date`) |
...
Колонка FSA `fullName` — это не продукция (у заявителя в карточке тоже есть `fullName`). Список отдаёт формулировку в `productFullName`. Поиск по номеру по-прежнему идёт в `number`.
