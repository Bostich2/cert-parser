...
- `limit_per_query` — сколько хитов оставить после объединения источников (дефолт `25`, диапазон 1…50). Это **размер выдачи**, не ограничение длины запроса. Запрос любой длины принимается, если после нормализации ≥ 4 символов.
...
На один запрос три провайдера вызываются параллельно (`asyncio.gather`): FSA сертификаты, FSA декларации и один OData ЕАЭС. С FSA не больше 10 записей с каждого списка; OData — до 20, затем ранжирование.

| `source` | Реестр | Поле поиска |
|----------|--------|-------------|
| `fsa_cert` | Росаккредитация, сертификаты (`pub.fsa.gov.ru`) | `columnsSearch.name = productFullName`; сортировка `date`; только «действует» и `endDate` от сегодня |
| `fsa_decl` | Росаккредитация, декларации | то же `productFullName`; сортировка `declDate`; те же статус и срок |
| `eaeu_ru` / `eaeu_other` | один OData-запрос к [tech.eaeunion.org](https://tech.eaeunion.org); `source` ставится по стране записи | `contains(p/productName, …)` в `technicalRegulationObjectDetails/productDetails` |

Колонка FSA `fullName` — это не продукция (у заявителя в карточке тоже есть `fullName`). Список отдаёт формулировку в `productFullName`. Поиск по номеру по-прежнему идёт в `number` **без** фильтров статуса и срока.

Категорий товара (ОКПД2 / полка магазина) в списке ФСА нет. Сужение — статус, срок действия и при таймауте технический регламент (`idTechReg` из `/identifiers` по ключевым словам вроде «шина» → ТР ТС 018/2011). ТН ВЭД в фильтре списка недоступен.

Дубликаты снимаются по компактному `official_number`. Ранжирование: сначала `country_code=RU`, затем вхождение нормализованной фразы в `product_name`, затем число совпавших токенов, затем действующий статус. Срез — `limit_per_query` (25 по умолчанию).

Нормализация запроса: минимум 4 символа после сжатия пробелов; иначе `query_too_short`. Ищется фрагмент формулировки из документа. Реестры не ограничивают длину поля 25 или 32 символами.

**32 — не лимит поля, а эвристика фразы.** Если токенов больше двух или нормализованная строка длиннее 32 символов, шаг `contains(вся строка)` пропускается: магазинский SKU в сертификате целиком не встречается и на OData/FSA даёт таймаут. Дальше OData — один `contains` по 1–2 буквенным словам (размеры вроде `235/65` отбрасываются), FSA — короткая подстрока вроде бренда и линейки. Таймаут шага не запускает следующий такой же запрос к тому же реестру; если по ключевым словам ясен ТР, FSA один раз повторяет тот же термин с `idTechReg`. Неудачный вход FSA не повторяется вторым провайдером сразу.
