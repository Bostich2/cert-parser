...
- **Поиск по товару** (beta) — по фрагменту формулировки продукции из действующих документов (не артикул магазина): сертификаты и декларации FSA плюс OData ЕАЭС.
...
### Вкладка «Поиск по товару» (beta)

Реестр ФСА заточен под проверку документа **по номеру**. Поиск по наименованию приблизительный: фрагмент формулировки из действующих сертификатов и деклараций, не артикул магазина. Лучше короткий запрос вроде «шины легковые Cordiant».
...
PDF в этой вкладке нет. Строки обрабатываются **последовательно**; по каждому имени источники на сервере идут параллельно.
...
- Поиск по товару: минимум 4 символа; FSA — только действующие документы, при таймауте повтор с техрегламентом; плюс OData ЕАЭС. Кэш lookup не используется.
