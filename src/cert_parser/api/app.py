...
from cert_parser.api.auth import (
    authenticate,
    ensure_csrf_token,
    get_session_user,
    is_admin,
...
from cert_parser.api.middleware import AuthMiddleware, SecurityHeadersMiddleware
from cert_parser.api.security import (
    get_client_address,
    read_upload_limited,
...
async def export_xlsx(request: Request, payload: ExportRequest) -> StreamingResponse:
    ...
    headers = {
        "Content-Disposition": 'attachment; filename="cert-parser-results.xlsx"',
    }
    return StreamingResponse(
        iter([content]),
